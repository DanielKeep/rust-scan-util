...
#[deriving(Show)]
pub struct NegInt(int);

impl Scanner<int> for NegInt {
	fn scan<'a, Tok: Tokenizer, Sp: Whitespace>(cursor: &Cursor<'a, Tok, Sp>) -> Result<(NegInt, Cursor<'a, Tok, Sp>), ScanError> {
		use std::from_str::FromStr;
		debug!("int.scan({})", cursor);
		...
		FromStr::from_str(int_str)
			.map(ref |i| Ok((NegInt(i), cursor.clone())))
			.unwrap_or_else(|| Err(OtherScanError(format!("expected integer, got `{}`", int_str), cursor.consumed())))
	}

	fn scanned_value(self) -> int {
		let NegInt(value) = self;
		-value
	}
}
